...
class ComptableDashboardEnhanced extends ConsumerStatefulWidget {
  ...
  static String title(BuildContext context) =>
      AppLocalizations.of(context)!.dashboardComptable;
...
class _ComptableDashboardEnhancedState
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(ComptableDashboardEnhanced.title(context)),
  ...
  Widget _buildStatisticsSection(
      BuildContext context, ComptableDashboardState state, {bool isWeb = false}) {
    final items = [
      _StatItem('Chiffre d\'affaires', state.totalRevenue, Icons.euro, const Color(0xFF059669), 'Montant total des factures'),
      _StatItem('Paiements reçus', state.totalPayments, Icons.payment, const Color(0xFF0D9488), 'Montant des paiements'),
      _StatItem('Dépenses total', state.totalExpenses, Icons.money_off, const Color(0xFFDC2626), 'Montant total des dépenses'),
      _StatItem('Salaires payés', state.totalSalaries, Icons.account_balance_wallet, const Color(0xFF7C3AED), 'Montant des salaires'),
      _StatItem('Bénéfice net', state.netProfit, Icons.trending_up, const Color(0xFF059669), 'Après dépenses'),
    ];
    if (isWeb) {
      final values = [state.totalRevenue, state.totalPayments, state.totalExpenses, state.totalSalaries, state.netProfit];
      final maxVal = values.fold<double>(0, (a, b) => b > a ? b : a);
      final scale = maxVal <= 0 ? 1.0 : maxVal;
      final colors = [const Color(0xFF059669), const Color(0xFF0D9488), const Color(0xFFDC2626), const Color(0xFF7C3AED), const Color(0xFF059669)];
      return Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Expanded(
            flex: 3,
            child: _buildComptableBarChart(context, state, scale, colors),
          ),
          ...
          SizedBox(
            ...
            child: Column(
              ...
              children: items.map((item) => Padding(
                ...
                child: _buildStatRowCompact(context, item),
              )).toList(),
            ),
          ),
        ],
  ...
  Widget _buildComptableBarChart(BuildContext context, ComptableDashboardState state, double scale, List<Color> colors) {
    const double minBarY = 0.04;
    final values = [state.totalRevenue, state.totalPayments, state.totalExpenses, state.totalSalaries, state.netProfit];
    final labels = ['CA', 'Paiem.', 'Dépens.', 'Salaires', 'Net'];
    double toY(double v) => (scale <= 0 ? 0.0 : (v / scale).clamp(0.0, 1.0));
        ...
        children: [
          ...
          SizedBox(
            ...
            child: state.isLoading
                ? Center(child: CircularProgressIndicator(color: ComptableDashboardEnhanced.primaryColor))
                : hasNoData
                        ...
                        builder: (ctx, constraints) => BarChart(
                          BarChartData(
                            ...
                            titlesData: FlTitlesData(
                              show: true,
                              bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, getTitlesWidget: (v, meta) => Text(labels[v.toInt().clamp(0, 4)], style: TextStyle(fontSize: 10, color: Colors.grey.shade600)))),
  ...
  Widget _buildStatRowCompact(BuildContext context, _StatItem item) {
    return Container(
  ...
  Future<void> _openCompanySelector(BuildContext context) async {
    Navigator.pop(context);
    final companiesAsync = ref.read(companiesProvider);
    final companies = companiesAsync.valueOrNull ?? await CompanyService.getCompanies();
    final currentId = ref.read(currentCompanyIdProvider);
    ...
    final selected = await showDialog<int>(
      ...
      builder: (ctx) {
        return AlertDialog(
          title: Text(l10n.currentCompany),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                RadioListTile<int>(
                  title: Text(l10n.allCompaniesMonoShort),
                  value: -1,
                  groupValue: currentId ?? -1,
                  onChanged: (v) => Navigator.pop(ctx, -1),
                ),
                ...companies.map((c) => RadioListTile<int>(
                  title: Text(c.name),
                  value: c.id,
                  groupValue: currentId ?? -1,
    ...
    ref.read(currentCompanyIdProvider.notifier).update((_) => id);
    if (context.mounted) {
      final msg = id == null ? l10n.companyAll : l10n.companyUpdated;
      ...
      child: Container(
        ...
        child: ListView(
          children: [
            DrawerHeader(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    l10n.roleLabel(Roles.getRoleName(userRole)),
            ...
            if (userRole == 1)
              _drawerItem(Icons.business_center, l10n.currentCompany,
                  DashboardEntityColors.parametres, () => _openCompanySelector(context)),
            _drawerItem(
                Icons.receipt, l10n.menuInvoices, DashboardEntityColors.factures,
                () => _nav(context, '/invoices')),
            _drawerItem(
                Icons.payment, l10n.menuPayments, DashboardEntityColors.paiements,
                () => _nav(context, '/payments')),
            _drawerItem(
                Icons.money_off, l10n.menuExpenses, DashboardEntityColors.expenses,
                () => _nav(context, '/expenses')),
            _drawerItem(
                Icons.account_balance_wallet, l10n.menuSalaries,
                DashboardEntityColors.salaries, () => _nav(context, '/salaries')),
            _drawerItem(Icons.book, l10n.menuJournal,
                DashboardEntityColors.journal, () => _nav(context, '/journal')),
            _drawerItem(Icons.menu_book, l10n.menuGrandLivre,
                DashboardEntityColors.grandLivre, () => _nav(context, '/comptable/grand-livre')),
            _drawerItem(Icons.balance, l10n.menuBalance,
                DashboardEntityColors.balance, () => _nav(context, '/comptable/balance')),
            _drawerItem(Icons.account_balance, l10n.menuTaxes,
                DashboardEntityColors.taxes, () => _nav(context, '/taxes')),
            _drawerItem(
                Icons.inventory, l10n.menuStock, DashboardEntityColors.stock,
                () => _nav(context, '/stocks')),
            _drawerItem(
                Icons.inventory_2, l10n.menuInventory,
                DashboardEntityColors.inventaire,
                () => _nav(context, '/stock/inventaire')),
            _drawerItem(Icons.business, l10n.menuSuppliers,
                DashboardEntityColors.fournisseurs,
                () => _nav(context, '/suppliers')),
            if (userRole == 1)
              _drawerItem(Icons.settings, l10n.settings,
            ...
            ListTile(
              ...
              title: Text(l10n.menuAttendance,
                  style: const TextStyle(color: Colors.white70)),
              onTap: () {
                Navigator.pop(context);
            ...
            ListTile(
              ...
              title:
                  Text(l10n.menuMyTasks, style: const TextStyle(color: Colors.white70)),
              onTap: () {
                Navigator.pop(context);
                context.go('/tasks');
              },
...
class _StatItem {
  ...
  final Color color;
  final String subtitle;
  _StatItem(this.title, this.value, this.icon, this.color, this.subtitle);
}
